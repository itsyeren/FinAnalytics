...
def fetch_ticker_info(ticker: str) -> dict:
    """yfinance üzerinden şirket bilgilerini çeker. TTL 1 saat."""
    try:
    ...
    st.markdown(
        ...
        ">
            ...
            <div style="
                ...
                padding:8px 16px;
            ">
                Desteklenen:  AAPL · MSFT · NVDA · GOOGL · AMZN · …
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
tabs = st.tabs(["Hakkında", "Kısa Vadeli Model", "Orta Vadeli Model", "Uzun Vadeli Model", "Haber Bülteni", "🤖 FinAI", "Raporlar"])
...
with tabs[0]:
    ...
    if not info:
        ...
        # ── Piyasa & Fiyat ─────────────────────────────────────────────────────────
        st.markdown("#### 📊 Piyasa & Fiyat Verileri")
        ...
        if officers:
            ...
            for idx, off in enumerate(officers):
                ...
                pay_str = f"Ücret: {_fmt_large(opay)}" if opay else ""
                with oc[idx % 3]:
                    st.markdown(
        ...
        if any(x is not None for x in [audit, board, comp_r, overall]):
            ...
            st.caption("ISP Governance Score — Düşük = daha az risk. Maksimum 10.")
        ...
        st.markdown("")
        st.caption(
            "⚠️ Veriler Yahoo Finance üzerinden çekilmektedir (TTL: 1 saat). "
...
with tabs[6]:
    ...
    st.caption(
        ...
        "Gündem Raporu Haber Bülteni haberlerinin özetlerini içerir."
    )
        ...
        if gen_fin:
            with st.spinner("Model verileri toplanıyor ve PDF oluşturuluyor…"):
                try:
                    ...
                    short_rows = []
                    h_labels = {"1d": "1 Gun", "3d": "3 Gun", "5d": "5 Gun", "7d": "7 Gun"}
