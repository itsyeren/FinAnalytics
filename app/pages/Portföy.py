...
SHORT_MODEL_DIR = ROOT_DIR / "models" / "short_term" / "src"
if str(SHORT_MODEL_DIR) not in sys.path:
    sys.path.insert(0, str(SHORT_MODEL_DIR))

# =========================================================
# IMPORTS (projeden)
# =========================================================
try:
    from config import TICKERS as ROOT_TICKERS, HORIZONS, DATA_DIR as KAGGLE_DATA_DIR
...
# Model dizinleri
SHORT_MODELS_DIR = ROOT_DIR / "models" / "short_term"
LONG_MODEL_PATH  = ROOT_DIR / "models" / "long_term" / "long_model.pkl"
ALL_STOCKS_CSV   = ROOT_DIR / "data" / "all_stocks.csv"
SECTOR_JSON      = ROOT_DIR / "models" / "mid_term" / "sector_optimized_params.json"
if not SECTOR_JSON.exists():
    SECTOR_JSON = ROOT_DIR / "models" / "mid_term" / "src" / "reports" / "sector_optimized_params.json"

# =========================================================
# PAGE CONFIG
# =========================================================
st.set_page_config(
    page_title="Portföy — FinAnalytics",
    layout="wide",
    page_icon="💼",
)
...
def _load_all_stocks_cache():
    ...
    if ALL_STOCKS_CSV.exists():
        ...
        return df
    # Fallback: Kaggle D1 dizininden oku (models/kisa_vadeli/kaynak/data_loader.py)
...
def load_short_model(ticker: str, horizon: int, algo: str):
    """Kısa vadeli model pkl dosyasını yükler."""
...
if report is None:
    ...
    if not mid_predictions:
        st.info("Portföydeki hisseler için orta vadeli model tahmini bulunamadı. (Yalnızca orta vadeli modelde tanımlı hisseler desteklenir)")
    else:
        # Metric cards
...
# latest_long'u scope dışında None olarak tanımla — Birleşik Tablo bölümünde güvenli erişim için
latest_long: pd.DataFrame | None = None
...
for t in tickers_in_portfolio:
    ...
    # Long
    if latest_long is not None and not latest_long.empty:
        lr = latest_long[latest_long["ticker"] == t]
        if not lr.empty:
